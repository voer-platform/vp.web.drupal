...
(function(window) {
  ...
  jQuery.fn.filterjs = function(data, view, options) {
  ...
  _FilterJS.prototype = {
    ...
    render: function(data, offset) {
      var $container = jQuery(this.container), record, el;

      if (!data) return;

      for (var i = 0, l = data.length; i < l; i++){
        record = this.getRecord(i, data);
        el = jQuery(this.view(record));
    ...
    bindEvents: function() {
      ...
      if (this.options.search){
        jQuery(this.options.search.input).on('keyup', function(e){
          self.filter();
        });
      }
    },
    ...
    bindSelectorEvent: function(selector, context) {
      jQuery(selector.element).on(selector.events, function(e) {
        context.filter();
      });
    },
    ...
    clear: function() {
      ...
      for (i; i < l; i++)
        jQuery(s[i].element).off(s[i].events);

      if (this.options.search) jQuery(this.options.search.input).off('keyup');
    ...
    filter: function(){
      ...
      for (i; i < l; i++){
        ...
        selected_vals = jQuery(s.element).filter(s.select).map(function() {
        ...
        if (selected_vals.length) {
          ...
          result = jQuery.grep((result || this.record_ids), function(v) {
    ...
    findObjects: function(category_vals, category_map, filter_type_func) {
      ...
      for (i; i < l; i++){
        ...
        if (filter_type_func){
          ids = jQuery.map(category_map, function(n,v){
    ...
    addFilterCriteria: function(name, criteria, ids_or_mapping) {
      ...
      ids_or_mapping = ids_or_mapping || jQuery(selector.element).data('ids') || [];
    ...
    parseSelectorOptions: function(selector, criteria) {
      ...
      var ele = jQuery(selector.element),
    ...
    hideShow: function(ids) {
      ...
      jQuery(this.container + ' > *[data-fjs]').hide();

      for (i; i < l; i++)
        jQuery(e_id + ids[i]).show();
    },

    search: function (search_config, filter_result) {
  		var val = jQuery.trim(jQuery(search_config.input).val());
	  	var search_in = search_config.search_in;
		  var min_length = jQuery.isNumeric(search_config.min_length) ? search_config.min_length : 1;
		  ...
		  return jQuery.map(filter_result, function (id) {
			  var $ele = jQuery(id_prefix + id);
    ...
    addData: function(data){
      ...
      //  if (jQuery(e_id + r.id).length == 0) uniq_data.push(data[i]);
      //}
    ...
    fetchData: function(){
      ...
      if (this.options.search) params['q'] = jQuery.trim(jQuery(this.options.search.input).val());

      jQuery.getJSON(opts.data_url, params).done(function(data){
